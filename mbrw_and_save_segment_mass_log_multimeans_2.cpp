...
run_params_t parse_args(const int argc, const char *argv[]) {
    ...
    for (size_t arg_index = 0; arg_index < argc; arg_index++) {
        ...
        else if (expect_order_file) {
            params.order_file = arg;
            found_order_file = true;
            expect_order_file = false;
        }
    ...
    // If no order directory is provided, assume the current directory.
    if (!found_order_file) {
        params.order_file = "orders.txt";
    }
...
bool read_orders(std::string file_name, std::vector<double> & orders) {
    bool is_valid;
    size_t num_orders = 0;
    std::ifstream order_file;
    std::string line;
    order_file.open(file_name);
    // std::cout << "reading orders list from " << file_name << "\n";
    if ( order_file.is_open() ) {
        ...
        // std::cout << "counting orders\n";
        is_valid = true;
        while ( std::getline(order_file, line) ) {
            // std::cout << line << "\n";
            num_orders++;
        }
        // std::cout << "found " << num_orders << " orders\n";
        // std::cout << "reading to order list\n";
        ...
        while ( std::getline(order_file, line) ) {
            std::cout << line << "\n";
            orders[order_index] = boost::lexical_cast<double>(line);
            order_index++;
        }
        order_file.close();
    }
...
int main(int argc, const char * argv[]) {
    ...
    bool read_orders_succeeded = read_orders( params.order_file, orders );
    if (!read_orders_succeeded) {
        std::cout << "Reading in order list failed.\n";
        return 1;
    }
    size_t num_orders = orders.size();
